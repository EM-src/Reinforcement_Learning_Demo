...
class Game:
    def __init__(self): #, width = 400, height = 400):
        ...
        self.walls = self.gameElements(level_map)[0]
        self.key = self.gameElements(level_map)[1]
        self.exit = self.gameElements(level_map)[2]
        self.player = self.gameElements(level_map)[3]
        pygame.draw.rect(self.screen, 'purple', self.player)
        self.fire = self.gameElements(level_map)[4]
        # self.walls, self.key, self.exit, self.player, self.fire = self.gameElements(level_map)
        # pygame.draw.rect(self.screen, 'purple', self.player)

        # self.playerPos = Pos(self.player.x, self.player.y)
        # self.allPos = []
    ...
    def move(self, direction):
        if direction != Direction.INITIAL_POSITION:
            self.move_count += 1
            #print('moves:', self.move_count)   # For debugging
    ...
    def collisions(self):
        if self.walls:
            for wall in self.walls:
                if self.player.colliderect(wall):
                    ...
                    #print('wall col:', self.collision_count)   # For debugging
        ...
        if self.exit:
            if self.player.colliderect(self.exit) and self.has_key == True:
